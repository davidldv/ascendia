...
function hashStringToSeed(value: string): number {
  // Simple deterministic hash; good enough for stable daily generation.
  let hash = 2166136261;
...
export function generateDailyMissions(params: {
  ...
  // Slow increase over time: stable per dateKey.
  const gentleRamp =
...
export function formatMissionTitle(type: MissionType): string {
  switch (type) {
    case "pushups":
      return "Push-ups";
    case "squats":
...
export function formatMissionTarget(
  ...
  if (type === "plank") return `${targetValue}s`;
  if (type === "run") return `${targetValue} min`;
